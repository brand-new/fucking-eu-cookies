# Fucking EU cookies (s odebranými výskyty "fucking" v produkčním kódu)
...
## Úpravy vzhledu
Vzhled lišty lze snadno upravit nastavením vlastností pro třídu `.eu-cookies`, abyste převážili výchozí nastavení, možná bude potřeba odkazovat se na `.eu-cookies.eu-cookies-priority`.
...
```
Obdobně můžete upravit i následující prvky: `.eu-cookies.eu-cookies-priority span` pro text, `.eu-cookies.eu-cookies-priority a` pro odkaz na *Více informací* a `.eu-cookies.eu-cookies-priority button` pro tlačítko.

## Úpravy funkčnosti
Lištu lze konfigurovat pomocí konfigurační proměnné `eu_cookies_config`.
...
Do Google Tag Manageru pak provolává event nazvaný `eu-cookies` se stejnými parametry jako Callback. Příklad:
```html
<script>
	var dataLayer = [];
	var eu_cookies_config = {
	...
	// Example event values: {event: 'eu-cookies', action: 'no-show', label: 'unsupported browser'}
</script>
```
